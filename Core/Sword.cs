...
{
    private readonly ScalingCalculator scalingCalculator;
    ...
    public ScalingAttribute[] Scaling { get; set; }

    public float GetModifiedAttack(Character attacker)
    {
        var modifiedPAttack = Damage;
        var attributes = attacker.CharacterPointsAttributes;

        modifiedPAttack += scalingCalculator.CalculateScaling(Scaling, attributes);
